...
# Class: GRQD.py
# Description: Game specfic logic and interaction for Reverse Google "Quick, Draw!"
...
class GRQDGame:
    ...
    def chooseObjects(self):
        with open('GRQDDep/categories.txt') as f:
            lines = [line.rstrip('\n') for line in open('GRQDDep/categories.txt')]
        GRQDGame.choices = random.sample(lines,4)
        GRQDGame.item = random.choice(GRQDGame.choices)

    def generateCoordinates(self):
        with open('GRQDDep/Drawing_Only/'+GRQDGame.item+'.ndjson') as f:
            objectLines = [line.rstrip('\n') for line in open('GRQDDep/Drawing_Only/'+GRQDGame.item+'.ndjson')]
